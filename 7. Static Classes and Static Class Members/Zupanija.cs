...
namespace OOP_Vj8
{
    class Zupanija
    {
        ...
        {
            //14.	Kod metode DodajNaselje prvo provjeriti ako već postoji naselje u listi
            // metoda u pozadini za svaki element liste naselja vrši usporedbu (pozivajući metodu Equals koja je
            // implementirana u razredu Naselje) sa instancom dobivenom kao argument metode: naselje
            // metoda Contains vraća true ako postoji barem 1 objekt koje ima iste vrijednosti (tj. metoda Equals)
            // a ako ne postoji vraća false što znači da možemo dodati naselje u listu
            if (naselja.Contains(naselje) == false)
                naselja.Add(naselje);
        }
        ...
        public string Opis
        {
            get
            {
                return $"{naziv} ima {BrojNaselja} naselja sa " +
                    $"{BrojStanovnika} stanovnika, prosječno " +
                    $"{ProsjecanBrojStanovnikaPoNaselju} stanovnika po naselju";
            }
        }

        //13.	Svojstvo ProsjecanBrojStanovnika
        public int ProsjecanBrojStanovnikaPoNaselju
        {
            get
            {
                return BrojStanovnika / BrojNaselja;
            }
        }
    }
}
